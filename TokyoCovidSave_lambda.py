...
def analyzeAndSave(tokyo, japan, call, local=False):   
    ...
    if (local):
        tokyoDemoAgg.to_json(DAILY_DEMO_JSON, orient='records')
    else:
        s3 = boto3.resource("s3")
        s3.Bucket(BUCKET_NAME).put_object(Key=DAILY_DEMO_JSON, Body=tokyoDemoAgg.to_json(orient='records'), ACL='public-read-write')
    ...
    if (local):
        with open(DAILY_DATA_JSON, 'w') as fp:
    ...
    if (local):
        dailyTrend.reset_index().to_json(DAILY_TREND_JSON, orient="records")
    else:
        s3 = boto3.resource("s3")
        s3.Bucket(BUCKET_NAME).put_object(Key=DAILY_TREND_JSON, Body=dailyTrend.reset_index().to_json(orient="records"), ACL='public-read-write')
